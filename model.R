...
model_func <- function(X,iterations){
  ...
  df <- data.frame(matrix(ncol = 7, nrow = iterations)) #holds AUC of each model each iter
  time_df <- data.frame(matrix(ncol = 7,nrow=iterations)) #hold training time
  acc_df <- data.frame(matrix(ncol = 7,nrow=iterations)) #hold accuracy
